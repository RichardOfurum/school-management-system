...
const SingleStudentPage = async ({
  ...
  | (Student & {
      class: Class & { _count: { lessons: number } };
      parent: Parent | null; // Include the parent relation
    })
  | null = await prisma.student.findUnique({
  where: { id },
  include: {
    class: { include: { _count: { select: { lessons: true } } } },
    parent: true, // Include the parent relation
  },
});
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap text-xs font-medium">
                ...
                {/* {role === "admin" && (
                  <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2">
                    ...
                    <span className="text-xs">{student.parent?.name + " " + student.parent?.surname}</span>
                  </div>
                )} */}
      ...
      <div className="w-full xl:w-1/3 flex flex-col gap-4">
        <div className="bg-white p-4 rounded-md">
          ...
          <div className="mt-4 flex gap-4 flex-wrap text-xs text-gray-500">
            ...
            <Link
              className="p-3 rounded-md bg-lamaPurpleLight"
              href={`/dashboard/list/attendance?studentId=${student.id}`}
            >
         ...
         
         {
          (role === "admin" || role === "teacher") && (
            student.parent && (
              <div className="bg-lamaPurpleLight py-6 px-4 rounded-md flex flex-col gap-4 h-fit">
                ...
                <div className="w-2/3 flex flex-col justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap text-xs font-medium">
                    ...
                    <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2">
                      <CiLocationOn />
                      <span className="text-xs">{student.parent.address || "-"}</span>
                    </div>
                  </div>
                </div>
              </div>
